...
async function scrapeJobs() {
  ...
  await page.goto("https://www.naukri.com/product-manager-jobs", { waitUntil: "domcontentloaded" });
  ...
  try {
    ...
    const jobs = await page.evaluate(() => {
      return [...document.querySelectorAll(".srp-jobtuple-wrapper")].map((job) => ({
        title: job.querySelector(".title a")?.innerText.trim() || "N/A",
        ...
        link: job.querySelector(".title a")?.href || "#",
      }));
    });
